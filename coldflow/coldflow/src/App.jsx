...
export default function App() {
  ...
  const [stepF, setStepF] = useState({day:1,time:'09:00',subj:'',body:'',condition:'none',waitDays:1})
  ...
  const [snippets, setSnippets] = useState([
    ...
    {id:'s3',name:'CTA - Calendar',category:'cta',text:'Here\'s my calendar if easier: {{calendlyLink}}'},
  ...
  useEffect(()=>{
    ...
    const handler=(e)=>{
      ...
      // Number keys 1-6 for nav (when no input focused)
      if(!e.metaKey&&!e.ctrlKey&&!e.altKey&&['1','2','3','4','5','6'].includes(e.key)&&document.activeElement.tagName!=='INPUT'&&document.activeElement.tagName!=='TEXTAREA'){
        const pages=['dashboard','leads','sequences','campaigns','analytics','settings']
  ...
  const addStep=async()=>{
    ...
    const ns={id:Date.now(),day:+stepF.day,time:stepF.time,subject:stepF.subj,body:stepF.body,condition:stepF.condition||'none',sent:0,opened:0,clicked:0,replied:0}
    const steps=[...(selSeq.steps||[]),ns].sort((a,b)=>a.day-b.day)
    await updateSeq(selSeq.id,{steps})
    setStepF({day:(steps.length||0)+1,time:'09:00',subj:'',body:'',condition:'none'});setEditingStepId(null);setModal(null)
  }

  const saveStep=async()=>{
    const steps=selSeq.steps.map(s=>s.id===editStep.id?{...s,day:+stepF.day,time:stepF.time,subject:stepF.subj,body:stepF.body,condition:stepF.condition||'none'}:s).sort((a,b)=>a.day-b.day)
    await updateSeq(selSeq.id,{steps})
    setEditStep(null);setEditingStepId(null);setStepF({day:1,time:'09:00',subj:'',body:'',condition:'none'});setModal(null)
  }
  ...
  const saveInlineStep=async()=>{
    ...
    if(editingStepId==='new'){
      ...
      const steps=selSeq.steps.map(s=>s.id===editingStepId?{...s,day:+stepF.day,time:stepF.time,subject:stepF.subj,body:stepF.body,condition:stepF.condition||'none'}:s).sort((a,b)=>a.day-b.day)
      await updateSeq(selSeq.id,{steps})
      setEditingStepId(null);setStepF({day:(selSeq.steps?.length||0)+1,time:'09:00',subj:'',body:'',condition:'none'})
    }
  }
  ...
  const saveAsTemplate=()=>{if(!stepF.subj.trim())return;setTemplates([...templates,{id:'t'+Date.now(),name:stepF.subj.slice(0,30),category:'Custom',subject:stepF.subj,body:stepF.body}])}
  ...
  const sendAi=async()=>{
    ...
    try{
      ...
      if(key){
        const res=await fetch('https://api.openai.com/v1/chat/completions',{
          ...
          body:JSON.stringify({model:'gpt-3.5-turbo',messages:[{role:'system',content:sysPrompt},...aiMsgs.map(m=>({role:m.role,content:m.content})),userMsg],max_tokens:500})
        })
        const data=await res.json()
        if(data.choices?.[0]?.message)setAiMsgs(prev=>[...prev,{role:'assistant',content:data.choices[0].message.content}])
      }else{
    ...
    }catch(e){setAiMsgs(prev=>[...prev,{role:'assistant',content:'Error. Try again.'}])}
    setAiLoad(false)
  }
  ...
  const GOOGLE_CLIENT_ID = '862994228270-igg8a6209esqe3nj3165rgqqk4u2m5ol.apps.googleusercontent.com'
  const GMAIL_SCOPES = [
  ...
  const connectCalendly = () => {
    const url = prompt('Enter your Calendly link (e.g., calendly.com/yourname):')
    if (url) {
  ...
  if(view==='landing')return(
    <div style={{minHeight:'100vh',background:'#000',color:'#fff',overflow:'hidden'}}>
      ...
      <section style={{textAlign:'center',padding:'100px 24px 80px',maxWidth:900,margin:'0 auto',position:'relative'}}>
        <div style={{display:'inline-flex',alignItems:'center',gap:10,padding:'10px 20px',background:'rgba(168,85,247,.1)',border:'1px solid rgba(168,85,247,.2)',borderRadius:100,marginBottom:40}}>
          <div style={{width:8,height:8,borderRadius:2,background:'#a855f7'}}/>
          <span style={{fontSize:14,color:'#a855f7',fontWeight:600}}>Integrates with Gmail</span>
        </div>
        ...
        <div style={{display:'flex',justifyContent:'center',alignItems:'center',gap:24}}>
          <div style={{display:'flex',alignItems:'center',gap:8}}><span style={{color:'#00ff88',fontSize:16}}>✓</span><span style={{color:'#666',fontSize:13}}>14 days free</span></div>
          ...
          <div style={{display:'flex',alignItems:'center',gap:8}}><span style={{color:'#00ff88',fontSize:16}}>✓</span><span style={{color:'#666',fontSize:13}}>Gmail integration</span></div>
        </div>
      </section>
      ...
      <section style={{padding:'0 24px 100px',maxWidth:1100,margin:'0 auto',position:'relative'}}>
        ...
        <div style={{background:'#0d0d0d',borderRadius:20,border:'1px solid rgba(168,85,247,.1)',overflow:'hidden',boxShadow:'0 0 60px rgba(168,85,247,0.1)',position:'relative'}}>
          <div style={{padding:'14px 20px',background:'#0a0a0a',borderBottom:'1px solid #262626',display:'flex',alignItems:'center',gap:12}}>
            ...
            <div style={{flex:1,textAlign:'center'}}><div style={{display:'inline-flex',alignItems:'center',gap:6,padding:'6px 14px',background:'#161616',borderRadius:8,border:'1px solid #262626'}}><span style={{fontSize:11,color:'#666'}}>🔒</span><span style={{fontSize:12,color:'#666'}}>app.coldflow.io</span></div></div>
            <div style={{width:80}}/>
          </div>
          <div style={{display:'grid',gridTemplateColumns:'200px 1fr',minHeight:420}}>
            ...
            <div style={{padding:28,background:'#000'}}>
              ...
              <div style={{background:'#0d0d0d',border:'1px solid rgba(168,85,247,.1)',borderRadius:14,padding:20}}>
                <div style={{fontSize:11,fontWeight:600,marginBottom:14,color:'#666',textTransform:'uppercase',letterSpacing:'0.05em'}}>Active Campaigns</div>
      ...
      <section id="features" style={{padding:'100px 24px',background:'#000'}}>
        <div style={{maxWidth:1100,margin:'0 auto'}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:20}}>
            {[
              {t:'Smart Lead Management',d:'Import leads from CSV, organize with tags, create dynamic segments.',icon:<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#a855f7" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>},
              ...
              {t:'Spam Score Checker',d:'Check deliverability before you send.',icon:<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#a855f7" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/><polyline points="9 12 11 14 15 10"/></svg>},
              {t:'Gmail Integration',d:'Connect your Gmail and start sending.',icon:<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#a855f7" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg>}
        ...
        <div style={{maxWidth:1100,margin:'0 auto'}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(2,1fr)',gap:20}}>
            {[
              ...
              {title:'Sales Leaders',desc:'Full visibility into team outreach. Know what\'s working without the meetings.',features:['Campaign performance analytics','Sequence-level conversion data','Export reports for any time range'],stat:'Complete visibility'}
      ...
      <section id="pricing" style={{padding:'100px 24px',background:'#000'}}>
        <div style={{maxWidth:1200,margin:'0 auto'}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:20,alignItems:'start'}}>
            {[
              {n:'Free',p:'$0',d:'Test the waters',f:['1,000 emails/month','1 sequence','1 inbox','Basic analytics','Community support'],pop:false,btn:'Get Started'},
              {n:'Solo',p:'$15',d:'For individual closers',f:['5,000 emails/month','Unlimited sequences','Unlimited campaigns','1 inbox','AI email writer','Spam checker','Email support'],pop:true,btn:'Start Free Trial'},
              {n:'Pro',p:'$49',d:'For high-volume senders',f:['25,000 emails/month','Unlimited sequences','Unlimited campaigns','1 inbox','A/B testing','Advanced analytics','Priority support'],pop:false,btn:'Start Free Trial'},
              {n:'Team',p:'Soon',d:'For growing teams',f:['Multiple seats','Inbox rotation','Shared sequences','Collision detection','Team analytics','Dedicated support'],pop:false,btn:'Join Waitlist',soon:true}
            ].map((x,i)=><div key={i} style={{background:'#0d0d0d',border:x.pop?'2px solid rgba(168,85,247,.4)':`1px solid rgba(168,85,247,.1)`,borderRadius:20,padding:32,position:'relative',boxShadow:x.pop?'0 0 40px rgba(168,85,247,0.15)':'0 0 25px rgba(168,85,247,0.05)',transition:'all .2s'}} onMouseOver={e=>{e.currentTarget.style.borderColor=x.pop?'rgba(168,85,247,.6)':'rgba(168,85,247,.3)';e.currentTarget.style.transform='translateY(-4px)'}} onMouseOut={e=>{e.currentTarget.style.borderColor=x.pop?'rgba(168,85,247,.4)':'rgba(168,85,247,.1)';e.currentTarget.style.transform='none'}}>
              ...
              <div style={{marginBottom:28}}>
                ...
                <div style={{fontSize:14,color:'#a3a3a3'}}>{x.d}</div>
              </div>
              <button onClick={()=>x.soon?null:setView('signup')} style={{width:'100%',padding:'14px',background:x.pop?'#a855f7':x.soon?'transparent':'rgba(168,85,247,.1)',color:x.pop?'#000':x.soon?'#666':'#a855f7',border:x.pop?'none':x.soon?'1px solid #262626':'1px solid rgba(168,85,247,.2)',borderRadius:100,fontSize:14,fontWeight:600,cursor:x.soon?'default':'pointer',marginBottom:28,opacity:x.soon?.6:1,boxShadow:x.pop?'0 0 30px rgba(168,85,247,0.3)':'none',transition:'all .2s'}} onMouseOver={e=>{if(!x.soon){e.target.style.transform='translateY(-2px)';if(x.pop)e.target.style.boxShadow='0 0 40px rgba(168,85,247,0.5)'}}} onMouseOut={e=>{if(!x.soon){e.target.style.transform='none';if(x.pop)e.target.style.boxShadow='0 0 30px rgba(168,85,247,0.3)'}}}>{x.btn}</button>
              <div style={{display:'flex',flexDirection:'column',gap:14}}>
                {x.f.map((f,j)=><div key={j} style={{display:'flex',alignItems:'center',gap:12}}>
                  <span style={{width:18,height:18,background:'rgba(168,85,247,.1)',borderRadius:6,display:'flex',alignItems:'center',justifyContent:'center',fontSize:10,color:'#a855f7',fontWeight:700}}>✓</span>
      ...
      <section style={{padding:'100px 24px',background:'#000'}}>
        <div style={{maxWidth:800,margin:'0 auto'}}>
          ...
          <div style={{display:'flex',flexDirection:'column',gap:12}}>
            {[
              {q:'How is ColdFlow different from Outreach or Salesloft?',a:'We built ColdFlow for individuals and small teams who want power without complexity. Same core features—sequences, campaigns, analytics—at a fraction of the price, with setup that takes minutes.'},
              {q:'Is there really a free plan?',a:'Yes. 1,000 emails/month, 1 sequence, 1 inbox. It\'s not a trial—it\'s free forever. No campaigns on free, but enough to test if ColdFlow fits your workflow.'},
              {q:'What\'s the difference between Solo and Pro?',a:'Volume. Solo gets you 5,000 emails/month for $15. Pro bumps that to 25,000 emails/month for $49, plus A/B testing and advanced analytics. Same features, more sending power.'},
              {q:'Do you offer email warmup?',a:'Built-in warmup is included in all paid plans. We gradually increase your sending volume to build sender reputation and avoid the spam folder.'},
              {q:'Can I import my leads from a spreadsheet?',a:'Export your leads as CSV and import directly. We handle standard formats and auto-map your columns.'},
      ...
      <div style={{flex:1,overflow:'auto',padding:32,transition:'all .2s',position:'relative',zIndex:1}}>
        ...
        {page==='dashboard'&&<>
          <div style={{marginBottom:40}}>
            ...
            <p style={{color:t.textSecondary,lineHeight:1.6,fontSize:15}}>Your outbound command center.</p>
          </div>
          <div style={{display:'grid',gridTemplateColumns:'repeat(5,1fr)',gap:16,marginBottom:32}}>{[{l:'LEADS',v:leads.length,primary:true},{l:'SENT',v:tSent},{l:'OPENED',v:tOpen,r:tSent?((tOpen/tSent)*100).toFixed(0)+'%':'0%'},{l:'CLICKED',v:tClick,r:tSent?((tClick/tSent)*100).toFixed(0)+'%':'0%'},{l:'REPLIED',v:tReply,r:tSent?((tReply/tSent)*100).toFixed(0)+'%':'0%',success:true}].map(s=><div key={s.l} style={{background:t.surface,border:t.cardBorder,borderRadius:16,padding:24,boxShadow:t.glow,transition:'all .2s',cursor:'default'}} onMouseOver={e=>{e.currentTarget.style.transform='translateY(-4px)';e.currentTarget.style.boxShadow=t.glowHover;e.currentTarget.style.borderColor=t.accent}} onMouseOut={e=>{e.currentTarget.style.transform='none';e.currentTarget.style.boxShadow=t.glow;e.currentTarget.style.borderColor='rgba(168,85,247,0.1)'}}><div style={{fontSize:11,color:t.textMuted,marginBottom:8,textTransform:'uppercase',letterSpacing:'0.05em',fontWeight:500}}>{s.l}</div><div style={{fontSize:32,fontWeight:800,color:s.primary?t.accent:s.success?t.success:t.text,letterSpacing:'-0.03em'}}>{s.v}</div>{s.r&&<div style={{fontSize:12,color:t.textMuted,marginTop:6}}>{s.r}</div>}</div>)}</div>
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:20}}>
            <div style={{background:t.surface,border:t.cardBorder,borderRadius:16,padding:24,boxShadow:t.glow,transition:'all .2s'}} onMouseOver={e=>{e.currentTarget.style.transform='translateY(-4px)';e.currentTarget.style.borderColor=t.accent}} onMouseOut={e=>{e.currentTarget.style.transform='none';e.currentTarget.style.borderColor='rgba(168,85,247,0.1)'}}><h3 style={{fontSize:11,fontWeight:600,marginBottom:20,textTransform:'uppercase',letterSpacing:'0.05em',color:t.textMuted}}>Active Campaigns</h3>{campaigns.filter(c=>c.status==='active').length===0?<p style={{color:t.textMuted,fontSize:14}}>No active campaigns yet.</p>:campaigns.filter(c=>c.status==='active').slice(0,5).map(c=><div key={c.id} onClick={()=>{setSelCamp(c);setPage('campaigns')}} style={{padding:16,background:t.elevated,borderRadius:12,marginBottom:8,cursor:'pointer',border:`1px solid ${t.border}`,transition:'all .15s'}} onMouseOver={e=>{e.currentTarget.style.borderColor=t.accent}} onMouseOut={e=>{e.currentTarget.style.borderColor=t.border}}><div style={{fontWeight:600,marginBottom:4,fontSize:14}}>{c.name}</div><div style={{fontSize:12,color:t.textMuted}}>{campLeads(c).length} leads · {c.stats?.sent||0} sent</div></div>)}</div>
            <div style={{background:t.surface,border:t.cardBorder,borderRadius:16,padding:24,boxShadow:t.glow,transition:'all .2s'}} onMouseOver={e=>{e.currentTarget.style.transform='translateY(-4px)';e.currentTarget.style.borderColor=t.accent}} onMouseOut={e=>{e.currentTarget.style.transform='none';e.currentTarget.style.borderColor='rgba(168,85,247,0.1)'}}><h3 style={{fontSize:11,fontWeight:600,marginBottom:20,textTransform:'uppercase',letterSpacing:'0.05em',color:t.textMuted}}>Recent Leads</h3>{leads.length===0?<p style={{color:t.textMuted,fontSize:14}}>No leads yet.</p>:leads.slice(0,5).map(l=><div key={l.id} style={{display:'flex',alignItems:'center',gap:12,padding:'10px 0',borderBottom:`1px solid ${t.border}`}}><div style={{width:36,height:36,borderRadius:10,background:`linear-gradient(135deg,${t.accent},#7c3aed)`,display:'flex',alignItems:'center',justifyContent:'center',fontSize:13,fontWeight:700,color:'#000'}}>{(l.first_name?.[0]||l.email[0]).toUpperCase()}</div><div><div style={{fontSize:14,fontWeight:600}}>{l.first_name} {l.last_name}</div><div style={{fontSize:12,color:t.textMuted}}>{l.company||l.email}</div></div></div>)}</div>
          </div>
        </>}
        ...
        {page==='leads'&&!selLead&&<>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:32}}>
            <div>
              ...
              <p style={{color:t.textSecondary,lineHeight:1.6,fontSize:15}}>Your prospect database. Import, organize, and track.</p>
            </div>
            <div style={{display:'flex',gap:10}}><Btn v="s" onClick={()=>setModal('import')}>Import CSV</Btn><Btn onClick={()=>setModal('lead')}>+ Add Lead</Btn></div>
          </div>
          ...
          <div style={{background:t.surface,border:t.cardBorder,borderRadius:16,overflow:'hidden',boxShadow:t.glow}}>
            ...
            {fLeads.map(l=><div key={l.id} style={{display:'grid',gridTemplateColumns:'40px 2fr 1fr 1fr 1fr 80px',gap:16,padding:'14px 20px',borderBottom:`1px solid ${t.border}`,alignItems:'center',cursor:'pointer',transition:'all .15s',background:selLeads.includes(l.id)?'rgba(168,85,247,.05)':'transparent'}} onMouseOver={e=>{if(!selLeads.includes(l.id))e.currentTarget.style.background='rgba(168,85,247,.03)'}} onMouseOut={e=>{if(!selLeads.includes(l.id))e.currentTarget.style.background='transparent'}}>
              ...
              <div onClick={()=>setSelLead(l)} style={{display:'flex',alignItems:'center',gap:12}}><div style={{width:40,height:40,borderRadius:12,background:`linear-gradient(135deg,${t.accent},#7c3aed)`,display:'flex',alignItems:'center',justifyContent:'center',fontSize:14,fontWeight:700,color:'#000'}}>{(l.first_name?.[0]||l.email[0]).toUpperCase()}</div><div><div style={{fontWeight:600,fontSize:14}}>{l.first_name} {l.last_name}</div><div style={{fontSize:12,color:t.textMuted}}>{l.email}</div></div></div>
        ...
        {page==='sequences'&&!selSeq&&<>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:32}}>
            <div>
              ...
              <p style={{color:t.textSecondary,lineHeight:1.6,fontSize:15}}>Multi-step email cadences that convert.</p>
            </div>
          ...
          {/* Existing Sequences */}
          {sequences.length>0&&<div style={{display:'flex',flexDirection:'column',gap:12}}>
            {sequences.map(s=>{
              const stepCount=s.steps?.length||0
              const totals=(s.steps||[]).reduce((acc,st)=>({sent:acc.sent+(st.sent||0),opened:acc.opened+(st.opened||0),replied:acc.replied+(st.replied||0)}),{sent:0,opened:0,replied:0})
              const campCount=campaigns.filter(c=>c.sequence_id===s.id).length
              return(
                <div key={s.id} onClick={()=>setSelSeq(s)} style={{background:t.surface,border:t.cardBorder,borderRadius:16,padding:24,cursor:'pointer',display:'flex',alignItems:'center',gap:20,transition:'all .2s',boxShadow:t.glow}} onMouseOver={e=>{e.currentTarget.style.transform='translateY(-2px)';e.currentTarget.style.borderColor=t.accent;e.currentTarget.style.boxShadow=t.glowHover}} onMouseOut={e=>{e.currentTarget.style.transform='none';e.currentTarget.style.borderColor='rgba(168,85,247,0.1)';e.currentTarget.style.boxShadow=t.glow}}>
                  <div style={{width:56,height:56,borderRadius:14,background:`linear-gradient(135deg,${t.accent},#7c3aed)`,display:'flex',alignItems:'center',justifyContent:'center',fontWeight:800,fontSize:20,flexShrink:0,color:'#000'}}>{stepCount}</div>
                  <div style={{flex:1,minWidth:0}}>
                    <div style={{fontSize:18,fontWeight:700,marginBottom:6,letterSpacing:'-0.02em'}}>{s.name}</div>
                    <div style={{display:'flex',gap:16,color:t.textMuted,fontSize:13}}>
                      <span>{stepCount} steps</span>
                      <span>{campCount} campaign{campCount!==1?'s':''}</span>
                      {totals.sent>0&&<span style={{color:t.accent}}>{totals.sent} sent</span>}
                    </div>
                  </div>
                  {totals.sent>0&&<div style={{display:'flex',gap:16,marginRight:8}}>
                    <div style={{textAlign:'center'}}><div style={{fontSize:16,fontWeight:700,color:t.text}}>{totals.sent?Math.round((totals.opened/totals.sent)*100):0}%</div><div style={{fontSize:10,color:t.textMuted,textTransform:'uppercase',letterSpacing:'0.05em'}}>Opens</div></div>
                    <div style={{textAlign:'center'}}><div style={{fontSize:16,fontWeight:700,color:t.success}}>{totals.sent?Math.round((totals.replied/totals.sent)*100):0}%</div><div style={{fontSize:10,color:t.textMuted,textTransform:'uppercase',letterSpacing:'0.05em'}}>Replies</div></div>
                  </div>}
                  <span style={{color:t.textMuted,fontSize:20}}>→</span>
                </div>
              )
            })}
            <button onClick={()=>setModal('sequence')} style={{padding:24,background:'transparent',border:`1px dashed ${t.border}`,borderRadius:16,color:t.textMuted,fontSize:14,cursor:'pointer',transition:'all .15s'}} onMouseOver={e=>{e.currentTarget.style.borderColor=t.accent;e.currentTarget.style.color=t.accent}} onMouseOut={e=>{e.currentTarget.style.borderColor=t.border;e.currentTarget.style.color=t.textMuted}}>+ Create New Sequence</button>
          </div>}
        </>}
        ...
        {page==='sequences'&&selSeq&&<>
          <div style={{display:'flex',gap:0,height:'calc(100vh - 96px)'}}>
            {/* Timeline Column */}
            <div style={{flex:1,display:'flex',flexDirection:'column',minWidth:0,maxWidth:editingStepId?'50%':'100%',transition:'max-width .2s'}}>
                ...
                <div style={{display:'flex',alignItems:'center',gap:12}}>
                  ...
                  <div>
                    ...
                    <div style={{display:'flex',alignItems:'center',gap:12,marginTop:6}}>
                      <span style={{color:'#666',fontSize:13}}>{selSeq.steps?.length||0} steps · {(selSeq.steps||[]).reduce((a,s)=>a+(s.day||0),0)} days</span>
                      ...
                      <span style={{fontSize:10,padding:'3px 8px',background:'rgba(34,197,94,.1)',border:'1px solid rgba(34,197,94,.25)',borderRadius:4,color:'#22c55e',fontWeight:600}}>🛑 STOPS ON REPLY</span>
                    </div>
                  </div>
                </div>
                <div style={{display:'flex',gap:8}}>
              ...
              <div style={{flex:1,overflow:'auto',paddingRight:16}}>
                <div style={{position:'relative',paddingLeft:28}}>
                  ...
                  {(selSeq.steps||[]).map((st,i)=>(
                    <div key={st.id} style={{position:'relative',marginBottom:8}}
                      draggable={editingStepId!==st.id}
                      onDragStart={()=>setDragIdx(i)}
                      onDragEnd={()=>{setDragIdx(null);setDragOverIdx(null)}}
                      onDragOver={e=>{e.preventDefault();setDragOverIdx(i)}}
                      onDrop={()=>reorderSteps(dragIdx,i)}>
                      {/* Timeline dot with condition indicator */}
                      <div style={{position:'absolute',left:-28,top:16,width:24,height:24,borderRadius:12,background:editingStepId===st.id?'#6366f1':st.condition&&st.condition!=='none'?'linear-gradient(135deg,#f59e0b,#ef4444)':i===0?'linear-gradient(135deg,#6366f1,#8b5cf6)':'#1e293b',border:editingStepId===st.id?'none':'2px solid #334155',display:'flex',alignItems:'center',justifyContent:'center',fontSize:11,fontWeight:700,color:editingStepId===st.id||i===0||st.condition&&st.condition!=='none'?'#fff':'#64748b',zIndex:1}}>{st.condition&&st.condition!=='none'?'⚡':i+1}</div>
                      
                      {/* Condition branch line */}
                      {st.condition&&st.condition!=='none'&&i>0&&(
                        <div style={{position:'absolute',left:-16,top:-4,width:12,height:20,borderLeft:'2px dashed rgba(245,158,11,.4)',borderBottom:'2px dashed rgba(245,158,11,.4)',borderBottomLeftRadius:8}}/>
                      )}
                      
                      {/* Step Card - Compact */}
                      <div 
                        onClick={()=>{setEditingStepId(st.id);setStepF({day:st.day,time:st.time,subj:st.subject,body:st.body,condition:st.condition||'none'});setPreviewStep(null)}}
                        style={{
                          background:editingStepId===st.id?'rgba(99,102,241,.1)':st.condition&&st.condition!=='none'?'rgba(245,158,11,.05)':dragOverIdx===i&&dragIdx!==i?'rgba(99,102,241,.08)':'rgba(255,255,255,.02)',
                          border:editingStepId===st.id?'1px solid rgba(99,102,241,.4)':st.condition&&st.condition!=='none'?'1px solid rgba(245,158,11,.25)':dragOverIdx===i&&dragIdx!==i?'1px dashed rgba(99,102,241,.4)':'1px solid rgba(255,255,255,.06)',
                          borderRadius:10,
                          padding:'12px 14px',
                          cursor:'pointer',
                        ...
                        {st.condition&&st.condition!=='none'&&(
                          <div style={{display:'flex',alignItems:'center',gap:6,marginBottom:8}}>
                            <span style={{fontSize:11,padding:'4px 10px',background:'rgba(245,158,11,.15)',border:'1px solid rgba(245,158,11,.35)',borderRadius:6,color:'#f59e0b',fontWeight:600,display:'flex',alignItems:'center',gap:5}}>
                              <span>⚡</span>
                              {st.condition==='if_opened'?'IF OPENED':st.condition==='if_not_opened'?'IF NOT OPENED':st.condition==='if_clicked'?'IF CLICKED':''}
                            </span>
                          </div>
                        )}
                        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:4}}>
                          ...
                          <div style={{display:'flex',alignItems:'center',gap:6}}>
                            {st.sent>0&&<span style={{fontSize:10,color:'#22c55e'}}>{Math.round((st.replied||0)/(st.sent||1)*100)}% reply</span>}
                            <span style={{cursor:'grab',color:'#444',fontSize:12}} onMouseDown={e=>e.stopPropagation()}>⋮</span>
                          </div>
                        </div>
                        <div style={{fontSize:13,fontWeight:500,color:'#e2e8f0',marginBottom:2,whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>{st.subject}</div>
                        <div style={{fontSize:12,color:'#64748b',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>{st.body?.substring(0,80)}...</div>
                      </div>
                    </div>
                  ))}
                        ...
                        {[
                          ...
                          <button key={preset.name} onClick={async()=>{
                            if(preset.steps.length===0){
                              setEditingStepId('new');setStepF({day:1,time:'09:00',subj:'',body:'',condition:'none'})
                            }else{
                  ...
                  <div style={{position:'relative'}}>
                    ...
                    <button 
                      onClick={()=>{setEditingStepId('new');setStepF({day:(selSeq.steps?.length||0)+1,time:'09:00',subj:'',body:'',condition:'none'})}}
            ...
            {editingStepId&&<div style={{width:'50%',borderLeft:'1px solid rgba(255,255,255,.08)',background:'rgba(0,0,0,.2)',display:'flex',flexDirection:'column',flexShrink:0}}>
              <div style={{padding:'16px 20px',borderBottom:'1px solid rgba(255,255,255,.08)',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                ...
                <button onClick={()=>{setEditingStepId(null);setStepF({day:(selSeq.steps?.length||0)+1,time:'09:00',subj:'',body:'',condition:'none'})}} style={{background:'rgba(255,255,255,.05)',border:'none',color:'#888',width:28,height:28,borderRadius:6,cursor:'pointer',fontSize:14}}>×</button>
              </div>
              ...
              <div style={{flex:1,overflow:'auto',padding:20}}>
                ...
                <div style={{marginBottom:16,padding:16,background:'rgba(99,102,241,.05)',border:'1px solid rgba(99,102,241,.15)',borderRadius:10}}>
                  ...
                  <div style={{display:'flex',flexDirection:'column',gap:8}}>
                    {[
                      ...
                      {id:'if_clicked',l:'If clicked',d:'Only send if a link in previous email was clicked',icon:'🔗'},
                    ].map(c=>(
                ...
                <div style={{marginBottom:16}}>
                  <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:6}}>
                    <label style={{fontSize:11,color:'#666',textTransform:'uppercase',letterSpacing:'0.5px'}}>Subject</label>
                    <button onClick={()=>{
                      const suggestions=[
                  ...
                  <input value={stepF.subj} onChange={e=>setStepF({...stepF,subj:e.target.value})} placeholder="Quick question about {{company}}" style={{width:'100%',padding:'10px 12px',background:'rgba(255,255,255,.05)',border:'1px solid rgba(255,255,255,.1)',borderRadius:8,color:'#fff',fontSize:14,boxSizing:'border-box'}}/>
                ...
                <div style={{marginBottom:16}}>
                  <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:6}}>
                    <label style={{fontSize:11,color:'#666',textTransform:'uppercase',letterSpacing:'0.5px'}}>Body</label>
                    <button onClick={()=>setAiOpen(true)} style={{padding:'4px 10px',background:'rgba(99,102,241,.15)',border:'none',borderRadius:5,color:'#6366f1',fontSize:11,cursor:'pointer',fontWeight:500}}>AI Help</button>
                  </div>
                  <textarea value={stepF.body} onChange={e=>setStepF({...stepF,body:e.target.value})} placeholder="Hey {{firstName}},&#10;&#10;Write your email here..." rows={12} style={{width:'100%',padding:12,background:'rgba(255,255,255,.05)',border:'1px solid rgba(255,255,255,.1)',borderRadius:8,color:'#fff',fontSize:13,boxSizing:'border-box',fontFamily:'inherit',resize:'vertical',lineHeight:1.6}}/>
                </div>
                ...
                <div style={{marginBottom:16,position:'relative'}}>
                  <div style={{display:'flex',gap:8}}>
                    ...
                    <button onClick={()=>{if(stepF.body.trim()){const name=prompt('Snippet name:');if(name){setSnippets([...snippets,{id:'s'+Date.now(),name,category:'cta',text:stepF.body}])}}}} style={{padding:'8px 14px',background:'transparent',border:'1px dashed rgba(255,255,255,.15)',borderRadius:8,color:'#666',fontSize:12,cursor:'pointer',fontWeight:500,display:'flex',alignItems:'center',gap:6}}>
              ...
              <div style={{padding:16,borderTop:'1px solid rgba(255,255,255,.08)',display:'flex',justifyContent:'space-between'}}>
                ...
                <div style={{display:'flex',gap:8}}>
                  <Btn v="g" s="s" onClick={()=>{setEditingStepId(null);setStepF({day:(selSeq.steps?.length||0)+1,time:'09:00',subj:'',body:'',condition:'none'})}}>Cancel</Btn>
        ...
        {page==='campaigns'&&selCamp&&<>
          ...
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'start',marginBottom:32}}>
            ...
            <div style={{display:'flex',gap:10}}><Btn v="s" onClick={()=>updateCamp(selCamp.id,{status:selCamp.status==='active'?'paused':'active'})}>{selCamp.status==='active'?'⏸ Pause':'▶️ Activate'}</Btn><Btn v="d" onClick={()=>delCamp(selCamp.id)}>Delete</Btn></div>
          </div>
        ...
        {page==='templates'&&<>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:32}}>
            <div>
              ...
              <p style={{color:t.textSecondary,lineHeight:1.6,fontSize:15}}>Reusable email templates for your sequences.</p>
            </div>
        ...
        {page==='snippets'&&<>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:32}}>
            <div>
              ...
              <p style={{color:t.textSecondary,lineHeight:1.6,fontSize:15}}>Reusable text blocks to speed up your writing.</p>
            </div>
        ...
        {page==='profile'&&<>
          <div style={{marginBottom:40}}>
            ...
            <p style={{color:t.textSecondary,lineHeight:1.6,fontSize:15}}>Your context for AI-powered personalization.</p>
          </div>
        ...
        {page==='analytics'&&<>
          <div style={{marginBottom:40}}>
            ...
            <p style={{color:t.textSecondary,lineHeight:1.6,fontSize:15}}>Performance insights across all campaigns.</p>
          </div>
        ...
        {page==='settings'&&<>
          <div style={{marginBottom:40}}>
            ...
            <p style={{color:t.textSecondary,lineHeight:1.6,fontSize:15}}>Integrations, delivery, and preferences.</p>
          </div>
              ...
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:16}}>
                ...
                <div style={{background:t.surface,border:t.cardBorder,borderRadius:14,padding:24,boxShadow:t.glow}}>
                  <div style={{display:'flex',alignItems:'center',gap:12,marginBottom:16}}>
                    ...
                    <div>
                      <h3 style={{fontSize:15,fontWeight:700,margin:0}}>Calendly</h3>
                      <p style={{fontSize:11,color:t.textMuted,margin:0}}>Auto-insert meeting links</p>
                    </div>
                  </div>
                  {settings.calendly.connected?
                    <div style={{display:'flex',alignItems:'center',justifyContent:'space-between'}}>
                      ...
                      <button onClick={()=>setSettings(s=>({...s,calendly:{connected:false,url:''}}))} style={{background:'none',border:'none',color:t.textMuted,fontSize:12,cursor:'pointer',textDecoration:'underline'}}>Disconnect</button>
                    </div>:
                    <Btn s="s" onClick={connectCalendly}>Connect Calendly</Btn>
                  }
                </div>
              </div>
            </div>
        ...
        <div style={{width:500,background:t.surface,border:t.cardBorder,borderRadius:16,overflow:'hidden',boxShadow:'0 0 60px rgba(168,85,247,0.1)'}} onClick={e=>e.stopPropagation()}>
          ...
          <div style={{maxHeight:350,overflow:'auto'}}>
            {[
              ...
              {icon:'→',label:'Go to Campaigns',shortcut:'4',action:()=>{setPage('campaigns');setCmdOpen(false)}},
              ...
              {icon:'+',label:'New Campaign',shortcut:'',action:()=>{setModal('campaign');setCmdOpen(false)}},
